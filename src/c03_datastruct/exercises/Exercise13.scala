...
import c03_datastruct.List.{foldLeft, foldRight, reverse}
...
object Exercise13 {
  ...
  /*
    ...
    performed with the correct associativity.
  ...
  def foldLeftViaFoldRight_1[A, B](as: List[A], outerIdent: B)(combiner: (B, A) => B): B = {
    ...
    /* foldLeft processes items in the reverse order from foldRight.

      Wrap each operation in a simple identity function to delay evaluation until later and stack (nest) the functions
      so that the order of application can be reversed.

      We'll call the type of this particular identity/delay function BtoB so we aren't writing B => B */
    type BtoB = B => B

    /* Here we declare a simple instance of BtoB according to the above description.  This function will be the identity
       value for the inner foldRight. */
    def innerIdent: BtoB = (b: B) => b

    /* For each item in the 'as' list (the 'a' parameter below), make a new delay function which will use the combiner
       function (passed in above) when it is evaluated later.  Each new function becomes the input to the previous
       function (delayFunc).
    ...
    /* Pass the original list 'as', plus the simple identity function and the new combinerDelayer to foldRight.
       This will create the functions for delayed evaluation with an combiner inside each one, but will not apply any
       of those functions. */
