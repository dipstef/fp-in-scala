...
object Reference extends Parsers[Parser] {
  ...
  // if the first parser consumes any characters, we ensure that the second parser is committed
  ...
  // In the event of failure, we want to push a new message onto the ParseError stack:

  // if scope(msg1)(a ** scope(msg2)(b)) fails while parsing b, the first error on the stack will be msg1, followed
  // by whatever errors were generated by a, then msg2, and finally errors generated by b.”
  ...
  def fail[A](msg: String): Parser[A] = s => Failure(s.loc.toError(msg), isCommitted = true)

  // The implementation of attempt just cancels the commitment of any failures that occur
